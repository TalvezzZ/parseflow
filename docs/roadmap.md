...
`v1.1.0` 已完成事件时间线、结构化日志、Prometheus/ready 诊断和真实 Docker runtime smoke；当前推进 `v1.2.0` 的质量报告与结果导出。
...
| `1.1.0` | 可观测性与运行验证 | 任务事件时间线、结构化日志、Prometheus、ready 诊断、Docker runtime smoke | 已完成并发布 |
