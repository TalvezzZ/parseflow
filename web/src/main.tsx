...
function App() {
  ...
  return <main className="app-shell">
    <header className="app-header"><span className="brand-mark" aria-hidden="true">✦</span><div><p className="eyebrow">PARSEFLOW · 1.0.0</p><h1>智能文档工作台</h1><p className="subtitle">上传一个文件，系统会自动规划并执行合适的解析流程。</p></div><span className="service"><i /> 服务就绪</span></header>
    <section className="workspace-grid">
      ...
      <section className="content-panel">{task ? <TaskView task={task} document={document} fileId={fileId} plans={plans} tables={tables} representations={representations} artifacts={artifacts} activeTab={activeTab} setActiveTab={setActiveTab} backToList={() => { setTask(null); setActiveTab('overview'); setError('') }} /> : <EmptyState recent={recent} open={(item) => { setTask(item); setActiveTab('overview'); setError('') }} />}</section>
    </section>
  </main>
}
...
function TaskView({ task, document, fileId, plans, tables, representations, artifacts, activeTab, setActiveTab, backToList }: { task: Task; document?: Json; fileId?: string; plans: Json[]; tables: Json[]; representations: Json; artifacts: Artifact[]; activeTab: string; setActiveTab: (tab: never) => void; backToList: () => void }) {
  ...
  const producedResult = task.status === 'succeeded' || task.status === 'partial'
  ...
  <div className="timeline"><Timeline label="任务已创建" state="done" /><Timeline label="自动规划" state={task.plan ? 'done' : task.status === 'queued' ? 'current' : 'waiting'} /><Timeline label="执行解析" state={task.status === 'running' ? 'current' : producedResult ? 'done' : 'waiting'} /><Timeline label="生成结果" state={producedResult ? 'done' : 'waiting'} /></div>
