...
async def test_single_upload_creates_plan_and_executes_xlsx(tmp_path: Path) -> None:
    ...
    response = await request("POST", "/api/v1/tasks/parse", files={"file": (source.name, source.read_bytes(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")})
...
async def test_single_upload_auto_routes_complex_rtf(tmp_path: Path) -> None:
    ...
    response = await request("POST", "/api/v1/tasks/parse", data={"goal": "提取文档内容", "data_id": "auto-rtf"}, files={"file": (source.name, source.read_bytes(), "application/rtf")})
    ...
    assert task["result"]["document"]["document_type"] in {"rtf", "docx"}
